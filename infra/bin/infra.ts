...
const repositoryName = app.node.tryGetContext("repositoryName") || "PawRush/sakai-ng";
const branchName = app.node.tryGetContext("branchName") || "deploy-to-aws";

// Manual deployment mode (preview/dev/prod stacks)
if (!codeConnectionArn) {
  ...
  const buildOutputPath = app.node.tryGetContext("buildPath") || "../dist/sakai-ng/browser";

  new FrontendStack(app, `SakaiNGFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
...
if (codeConnectionArn) {
  new PipelineStack(app, "SakaiNGPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for SakaiNG",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "SakaiNG");
